function day4(resultElement) {
    ...
    function findSolution2(list) {
        var validators = {
            'byr': (x) => { return 1920 <= x && x <= 2002 },
            'iyr': (x) => { return 2010 <= x && x <= 2020 },
            'eyr': (x) => { return 2020 <= x && x <= 2030 },
            'hgt': (x) => {
                if (x.indexOf("cm") != -1) {
                    var val = parseInt(x.replace("cm", ""));
                    return 150 <= val && val <= 193;
                } if (x.indexOf("in") != -1) {
                    var val = parseInt(x.replace("in", ""));
                    return 59 <= val && val <= 76;
                }
                return false;
            },
            'hcl': (x) => { return x.match(/^#[a-z0-9]{6}$/gm) },
            'ecl': (x) => {
                var colors = ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'];
                return colors.indexOf(x) != -1;
            },
            'pid': (x) => {
                return x.match(/^\d{9}$/gm)
            }
        };
    ...
    function getTestList() {
        return ['ecl:gry pid:860033327 eyr:2020 hcl:#fffffd byr:1937 iyr:2017 cid:147 hgt:183cm', 'iyr:2013 ecl:amb cid:350 eyr:2023 pid:028048884 hcl:#cfa07d byr:1929', 'hcl:#ae17e1 iyr:2013 eyr:2024 ecl:brn pid:760753108 byr:1931 hgt:179cm', 'hcl:#cfa07d eyr:2025 pid:166559648 iyr:2011 ecl:brn hgt:59in'];
    }

    function getList() {
        return ['eyr:2027 hcl:#602927 hgt:186cm byr:1939 iyr:2019 pid:552194973 ecl:hzl',
