function day2(resultElement) {
    function pwdIsValidPart1(line) {
        var [policy, pwd] = line.split(":");
        var [policyTimes, char] = policy.split(" ");
        var [min,max] = policyTimes.split("-");

        var count = pwd.split(char).length-1;
        return min <= count && count <= max;
    }

    function pwdIsValidPart2(line) {
        var [policy, pwd] = line.split(":");
        var [policyPosition, char] = policy.split(" ");
        var [pos1,pos2] = policyPosition.split("-");
        pwd = pwd.trim();
        var pos1ContainsLetter = pwd[pos1-1] == char;
        var pos2ContainsLetter = pwd[pos2-1] == char;
        return pos1ContainsLetter ^ pos2ContainsLetter;
    }
    ...
    for (var i = 0; i < list.length; i++) {
        if (pwdIsValidPart1(list[i])) {
            correctPasswordsPart1++;
        }
    }
    var correctPasswordsPart2 = 0;
    for (var i = 0; i < list.length; i++) {
        if (pwdIsValidPart2(list[i])) {
            correctPasswordsPart2++;
        }
    }

    resultElement.innerHTML = `Correct passwords for part 1: ${correctPasswordsPart1}<br>Correct passwords for part 2: ${correctPasswordsPart2}`;
